...
const ReadingLesson: React.FC<ReadingLessonProps> = ({ lessonId }) => {
  ...
  useEffect(() => {
    let cancelled = false; (async () => {
      if (!lessonId) { setError("Thiếu mã bài học."); setLoading(false); return; } setLoading(true); setError(null);
  ...
  useEffect(() => {
    let cancelled = false; (async () => {
      if (!selectedDocId) return; try { setLoading(true); const doc = await apiService.getReadingDocDetail(selectedDocId); if (cancelled) return; setDocDetail(doc); }
